...
- **No recent changes**: If there are no merged PRs in the last 24 hours, call the safe-outputs `noop` tool with a brief summary
- **Already documented**: If all features are already documented, call the safe-outputs `noop` tool with a brief summary
...
- If no PR is created, you MUST still emit exactly one safe output by calling `noop` (or `missing_tool` / `missing_data` when blocked)
