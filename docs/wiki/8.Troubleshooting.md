...
- ✅ Smart Paste dependencies when `links.smart_paste.enabled = true` (`curl` executable + Neovim 0.11+)
...
1. Run the health check — it includes a dedicated **Smart Paste** section when the feature is enabled:
   ...
   :checkhealth markdown-plus
   ```
   It checks for `curl` (required to fetch page titles) and that Neovim 0.11+ is installed.
