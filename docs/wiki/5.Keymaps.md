...
### Toggle list type (normal + visual)
...
| `<localleader>ltp` | Toggle lowercase letter paren list (`a)`) |
| `<localleader>ltP` | Toggle uppercase letter paren list (`A)`) |
| `<localleader>ltc` | Clear list markers (plain text) |

> [!TIP]
> Each `<localleader>lt*` key toggles the type on/off: pressing the same key when the line already matches
> the target type reverts it to plain text. For an interactive one-key picker, bind
> `<Plug>(MarkdownPlusToggleListPick)` to `<localleader>lt`; it waits for a single keypress (`u/t/n/N/l/L/p/P/c`).
> which-key users see the full menu after typing `<localleader>lt`.
