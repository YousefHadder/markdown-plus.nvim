...
Toggle the current line or selection to a list type. Plain text gains a marker; a line already of the target type is cleared back to plain text; a different type is converted.

| Keymap | Description | Marker |
|---|---|---|
| `<localleader>ltu` | Toggle unordered list | `- ` |
| `<localleader>ltt` | Toggle task list | `- [ ] ` |
| `<localleader>ltn` | Toggle ordered list | `1.` |
| `<localleader>ltN` | Toggle parenthesized ordered list | `1)` |
| `<localleader>ltl` | Toggle lowercase letter list | `a.` |
| `<localleader>ltL` | Toggle uppercase letter list | `A.` |
| `<localleader>ltp` | Toggle parenthesized lowercase letter list | `a)` |
| `<localleader>ltP` | Toggle parenthesized uppercase letter list | `A)` |
| `<localleader>ltc` | Clear list markers (plain text) | — |

> [!TIP]
> Because these are real buffer-local mappings, plugins like **which-key.nvim** discover them automatically and show the menu after you press `<localleader>lt`.
>
> If you prefer a picker that waits indefinitely for the type key, bind it yourself:
> ```lua
