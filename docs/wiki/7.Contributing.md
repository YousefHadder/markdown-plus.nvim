...
└── markdown-plus/             # 49 spec files
    ...
    ├── keymap/                # Internal keymap_fallback submodules
    │   ├── feed_spec.lua      #   key feeding
    │   ├── guard_spec.lua     #   recursion guards
    │   └── resolve_spec.lua   #   mapping resolution
...
`lua/markdown-plus/keymap_fallback.lua` is a thin facade over the
`lua/markdown-plus/keymap/` submodules (`guard.lua`, `resolve.lua`, `feed.lua`,
`types.lua`) — recursion guards, mapping resolution, key feeding, and shared
types split into separate files, each under 200 lines. The public `run`,
`resolve`, and `reset` entry points are unchanged; keep using
`require("markdown-plus.keymap_fallback")`.
