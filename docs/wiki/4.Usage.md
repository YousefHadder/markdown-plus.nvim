...
Use `<localleader>lt` followed by a type key to convert any line or visual selection to a different list style. Plain text gains a marker; the same type clears back to plain text; a different type converts in place.

```markdown
" In normal mode – position cursor on a line, then:
write README           →  <localleader>ltt  →  - [ ] write README  (task list)
1. [x] ship            →  <localleader>ltu  →  - ship               (unordered, drops checkbox)
- ship                 →  <localleader>ltn  →  1. ship              (numbered)
- [ ] draft            →  <localleader>ltl  →  a. [ ] draft         (lowercase letter, keeps checkbox)
- item                 →  <localleader>ltc  →  item                 (clear – plain text)

" In visual mode – select multiple lines, then apply a type key:
- [ ] Task A           →  <localleader>ltn  →  1. [ ] Task A
- [ ] Task B                                →  2. [ ] Task B
- [ ] Task C                                →  3. [ ] Task C
" (ordered lists are automatically renumbered per indentation group)
```

Key reference:

| Key suffix | Type | Example |
|---|---|---|
| `u` | Unordered | `- item` |
| `t` | Task | `- [ ] item` |
| `n` | Numbered | `1. item` |
| `N` | Numbered paren | `1) item` |
| `l` | Lowercase letter | `a. item` |
| `L` | Uppercase letter | `A. item` |
| `p` | Lowercase paren | `a) item` |
| `P` | Uppercase paren | `A) item` |
| `c` | Clear | plain text |

> [!TIP]
> which-key.nvim users: the `<localleader>lt` prefix is a real set of buffer-local mappings, so which-key will display the menu automatically.
