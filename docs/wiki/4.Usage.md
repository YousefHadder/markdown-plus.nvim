...
Press `<localleader>lt` followed by a type key to convert the current line (normal mode) or selection
(visual mode) to the target list type. Pressing the same type key when all lines already match that
type clears the markers instead (toggle-off).
...
| `c` | clear (strip all list markers) |

```markdown
- write README        <localleader>ltt  →  - [ ] write README
1. [x] ship           <localleader>ltu  →  - ship        (checkbox dropped on unordered)
- [x] ship            <localleader>ltn  →  1. [x] ship   (checkbox preserved on ordered)

Visual selection (all already unordered → toggles off):
- apple               <localleader>ltu  →  apple
- banana                               →  banana
```

> [!TIP]
> which-key.nvim shows the type menu automatically after `<localleader>lt`.
> Prefer a picker that waits indefinitely? Bind `<Plug>(MarkdownPlusToggleListPick)` to your key:
> ```lua
