...
Use the `<localleader>lt` prefix to convert the current line (or a visual selection) between any list type. A line already of the target type is cleared back to plain text; a line of a different type is converted; plain text gains a marker.

```markdown
Press <localleader>ltu to toggle unordered:
  write README            →  - write README     (plain → unordered)
  - write README          →  write README        (same type → cleared)
  1. write README         →  - write README      (different type → converted)

Press <localleader>ltt to toggle task list:
  write README            →  - [ ] write README
  - write README          →  - [ ] write README
  1. [x] ship it          →  - [ ] ship it       (checkbox preserved)

Press <localleader>ltn to toggle numbered:
  write README            →  1. write README
  - [x] ship it           →  1. [x] ship it      (checkbox preserved)
  1. write README         →  write README         (same type → cleared)

Press <localleader>ltu on a visual selection:
  - [ ] Task 1            →  Task 1              (all unordered → cleared)
  - Task 2                →  Task 2

Press <localleader>ltn on a visual selection of mixed types:
  - item one              →  1. item one         (each line converted)
  plain text              →  2. plain text        (renumbered per group)
  - item two              →  3. item two
```

All `<localleader>lt_` mappings work in both normal and visual mode. Because they are real buffer-local keymaps, `which-key.nvim` shows the menu automatically after pressing `<localleader>lt`. The second key must be pressed within `'timeoutlen'`.
