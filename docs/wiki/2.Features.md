...
- Configurable marker-to-content whitespace: `list.whitespace = "shiftwidth"` (with `list.whitespace_width`) aligns list content to a fixed-width column block, keeping continuation lines aligned; default is `"single"` (one space, standard Markdown)
