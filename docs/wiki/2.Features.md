...
- Configurable marker-to-content whitespace: `list.whitespace = "shiftwidth"` (with `list.whitespace_width`)
  aligns list content to a fixed-width column block, keeping continuation lines aligned; default is "single"
  (one space, standard Markdown)
- Optional checkbox completion timestamps (emoji/comment/dataview/parenthetical)
- **List type toggling** via the `<localleader>lt` prefix — press the prefix then a type key to convert the
  current line or visual selection: `u` unordered, `t` task, `n` numbered (`1.`), `N` numbered-paren (`1)`),
  `l` lowercase letter (`a.`), `L` uppercase letter (`A.`), `p` lowercase-paren (`a)`), `P` uppercase-paren
  (`A)`), `c` clear (plain text). Works in normal and visual modes; every type also has a dedicated `<Plug>`
  mapping. which-key.nvim discovers the prefix automatically. For an indefinitely-waiting picker, bind
  `<Plug>(MarkdownPlusToggleListPick)` to your preferred key.
