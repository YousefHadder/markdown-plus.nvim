...
- **List type toggling**: convert any line/selection between unordered, task, ordered, parenthesized, or letter-based list types via the `<localleader>lt` prefix — `which-key` compatible, plus a `<Plug>(MarkdownPlusToggleListPick)` picker variant
