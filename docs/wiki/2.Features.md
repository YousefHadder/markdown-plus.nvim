...
- **Toggle list type** (`<localleader>lt` prefix): switch the current line or selection between unordered (`u`),
  task (`t`), ordered `1.` (`n`), ordered `1)` (`N`), `a.` (`l`), `A.` (`L`), `a)` (`p`), `A)` (`P`), or clear
  markers (`c`). Each type also has its own `<Plug>` mapping; bind `<Plug>(MarkdownPlusToggleListPick)` to
  `<localleader>lt` for an indefinitely-waiting picker instead of nested keymaps.
