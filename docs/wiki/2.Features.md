...
- **List type toggling** (`<localleader>lt` prefix): convert a line or selection to unordered, task, numbered (`1.`/`1)`), or letter-based (`a.`/`A.`/`a)`/`A)`) lists; clear back to plain text with `<localleader>ltc`; which-key compatible
