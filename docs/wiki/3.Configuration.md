...
require("markdown-plus").setup({
  ...
  list = {
    ...
    whitespace = "single",        -- "single" | "shiftwidth"
                                  ...
                                  -- "shiftwidth": pad to align content to a whitespace_width column block
...
- Replace old `<leader>` defaults with `<localleader>`.
- If desired, set localleader before plugin setup:

```lua
vim.g.maplocalleader = "\\"
```

- If you integrate with internal header parsing, update calls to:
...
- [v2.0 Migration Guide](https://github.com/YousefHadder/markdown-plus.nvim/wiki/9.v2.0-Migration-Guide)
