...
There is no dedicated `<A-CR>` kind. Use `in_list_context("enter")` when composing an `<A-CR>`
mapping: it recognizes marker lines and continuation lines, matching `continue_list_content`.
`"indent"` is for `<Tab>`/`<S-Tab>` and only recognizes marker lines.

Continuation recognition looks back at most 20 lines for the item's marker. Starting on
`- item` and typing content after each `<A-CR>`, presses 1-21 add continuation lines; press 22
falls back to the other `<A-CR>` mapping (or native behavior) because the marker is now 21
lines above the cursor. This inherited bound keeps per-key scanning bounded, but does not
support arbitrarily long items. The `"enter"` predicate shares this limit.
