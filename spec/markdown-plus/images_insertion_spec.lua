...
describe("markdown-plus images", function()
  ...
  describe("get_image_at_cursor", function()
    it("returns image with title", function()
      vim.api.nvim_buf_set_lines(0, 0, -1, false, { '![alt](https://img.png "My title")' })
      vim.api.nvim_win_set_cursor(0, { 1, 5 })

      local result = images.get_image_at_cursor()
      assert.is_not_nil(result)
      assert.equals("alt", result.alt)
      assert.equals("https://img.png", result.url)
      assert.equals("My title", result.title)
    end)
  end)

  describe("insert_image without title", function()
    it("inserts image without title when title is empty", function()
      vim.api.nvim_buf_set_lines(0, 0, -1, false, { "" })
      vim.api.nvim_win_set_cursor(0, { 1, 0 })

      input_spy = mocks.mock_input({ "logo", "https://logo.png", "" })

      images.insert_image()

      local lines = vim.api.nvim_buf_get_lines(0, 0, -1, false)
      assert.equals("![logo](https://logo.png)", lines[1])
    end)
  end)

  describe("setup_keymaps", function()
    it("registers plug mappings via enable", function()
      images.setup({ enabled = true, keymaps = { enabled = true } })
      images.enable()
      assert.equals(1, vim.fn.hasmapto("<Plug>(MarkdownPlusInsertImage)", "n"))
    end)
  end)
end)
